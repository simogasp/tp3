...
def main(working_dir: str, archive_name: str, skip_cleaning: bool = False, skip_packaging: bool = False):
    ...
    files_to_copy = ["CMakeLists.txt", "README.md", "LICENSE", "lumiere.cpp", ".clang-format"]
    ...
    lumiere_cpp = os.path.join(dest_name, "lumiere.cpp")
    logger.info("Applying studentify to %s", lumiere_cpp)
    subprocess.run(
        ["python3", os.path.join(studentify_dir, "studentify.py"), lumiere_cpp, "-o", lumiere_cpp, "--force"])
